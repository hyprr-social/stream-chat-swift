...
open class ChatMessageActionsVC<ExtraData: ExtraDataTypes>: ViewController, UIConfigProvider {
    ...
    open var messageActions: [ChatMessageActionItem] {
        ...
        let editAction = ChatMessageActionItem(name: .edit) { [weak self] in self?.handleEditAction() }
        let deleteAction = ChatMessageActionItem(name: .delete) { [weak self] in self?.handleDeleteAction() }

        switch message.localState {
        case nil:
            var actions: [ChatMessageActionItem] = [
                .init(name: .inlineReply) { [weak self] in self?.handleInlineReplyAction() },
                .init(name:.threadReply) { [weak self] in self?.handleThreadReplyAction() },
                .init(name:.copy) { [weak self] in self?.handleCopyAction() }
            ]

            if message.isSentByCurrentUser {
                actions += [editAction, deleteAction]
            } else if currentUser.mutedUsers.contains(message.author) {
                actions.append(.init(name:.unmuteUser) { [weak self] in self?.handleUnmuteAuthorAction() })
            } else {
                actions.append(.init(name:.muteUser) { [weak self] in self?.handleMuteAuthorAction() })
            }

            return actions
        case .pendingSend, .sendingFailed, .pendingSync, .syncingFailed, .deletingFailed:
            return [
                message.localState == .sendingFailed ? .init(name: .resend) { [weak self] in self?.handleResendAction() } : nil,
                editAction,
                deleteAction
            ].compactMap { $0 }
